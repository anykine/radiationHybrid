...
use strict;
#
# PURPOSE: compare RAT T55 with MOUSE T31
...
for (my $count =0 ; $count<=$#datafile; $count++){
	...
	for (my $count1 = 0; $count1 <2; $count1++){
		...
		my $mousegeneHashRef = getMouseGene($ratgenesArrayRef);
...
sub getRatMarker {
	my($start, $end) = @_;
	my @ratpos= ();
	my $SQL_GET_LOC_IN_RAT="select a.sts_name,a.m_order,b.trueName,c.chrom,c.chromStart,c.chromEnd from rat_rh.stsname_marker_link a JOIN rat_rh.stsAlias b ON a.sts_name = b.alias JOIN rat_rh.stsMapRat c ON b.trueName = c.name where m_order between ? and ?";
...
sub getRatGene {
	my($valsRef) = @_;
	my $SQL_GET_GENE_RAT = "SELECT a.kgID, a.geneSymbol, b.chrom, b.txStart FROM rat_rh.kgXref a JOIN rat_rh.knownGene b on a.kgID = b.name WHERE b.chrom=? and txStart > ? and txStart < ? ORDER by b.txStart";
...
# given list of rat genes, see if such a mouse gene exist
...
sub getMouseGene {
	...
	my($where, $listRef) = buildGetGeneMouse($ratgenesArrayRef);
...
# note: some mouse genes are on diff chromosomes (in rat, same chrom)
...
sub minmax2 {
	...
	#some mouse genes are on diff chromosomes (in rat, same chrom)
	# need to get min/max for genes on each chrom (assuming synteny)
