...
int main()
{
	...
	char upret_file[] = "for_C_scaled_cgh.txt";
	char downexp_file[] = "for_C_downstream_expression_log10.txt";
	char extreme_file[] = "extreme_retention_markers.txt";
	// ouptut file
	char output_file[] = "alpha_null_counts_rm_ex_4.txt";
...
printf ("Loading Input files ... \n");
	int count = 0;
	int extremes[336];
	if (!(fe = fopen(extreme_file, "r")))
	{
		printf ("Error opening extreme file\n");
	}
	for (i = 0; i < 336; i++)
	{
		fscanf(fe, "%i ", &extremes[i]);
	} 
	fclose(fe);
printf ("Read extreme file\n");
...
for (gene = 12950; gene < 14389; gene++ ) { 
	printf("%i\n", gene);
	...
	for (marker = 0; marker < uprows; marker++) {
		...
		for (k=0; k<5; k++ ) {
			...
			gsl_fit_linear(cgh, 1, dperm, 1, 99, &mu, &alpha, &cov00, &cov01, &cov11, &sumsq);
