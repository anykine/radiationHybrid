...
int main()
{
	...
	char upret_file[] = "/home3/ANOVA_CGH/input/for_C_scaled_cgh.txt";
	char downexp_file[] = "/home3/ANOVA_CGH/input/for_C_downstream_expression_log10.txt";
...
for (gene = 0; gene < 10; gene++ ) { 
	...
	for (marker = 0; marker < uprows; marker++) {
		...
		gsl_fit_linear(cgh, 1, expr, 1, 99, &mu, &alpha, &cov00, &cov01, &cov11, &sumsq);
