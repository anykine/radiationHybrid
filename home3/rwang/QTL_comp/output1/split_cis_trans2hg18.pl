...
# 6/11/08 - split linear model into cis and trans based on hg18
#
...
sub split_cistrans{
	...
	open(INPUT, 'g3alpha_model_results1.txt') || die "cannot open results\n";
	#open(CISOUT, '>g3alpha_model_results1_gt4cis.txt') || die "cannot open cis for write\n";
	open(CISOUT, '>/media/G3data/fdr18/g3alpha_model_results1_cis.txt') || die "cannot open cis for write\n";
	#open(TRANSOUT, '>g3alpha_model_results1_gt4trans.txt') || die "cannot open trans for write\n";
	open(TRANSOUT, '>/media/G3data/fdr18/g3alpha_model_results1_trans.txt') || die "cannot open trans for write\n";
	while(<INPUT>){
		chomp;
