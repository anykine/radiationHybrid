...
// to run: GSL_RNG_TYPE="taus" GSL_RNG_SEED=43 LD_LIBRARY_PATH=/usr/local/lib
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
//int BINCNT=10000000O0;
int BINCNT=1000000;

int main()
{
	// input files for upstream retention and downstream expression
	char upret_file[] = "cgh_ret.txt";
	char downexp_file[] = "exp.txt";

	// ouptut file
	char output_file[] = "histogram_of_null_alphas.txt";
...
printf ("Loading Input files ... \n");
	...
	for (row = 0; row < downrows; row++)
	{
		...
		while (pch != NULL) 
		{									
			downexp[row][col]=log10(atof(pch));
			pch = strtok (NULL, "\t\n");
			col++;
		}
	}
...
printf("now calculating full model SSRF\n");
	...
	for (marker = 0+5; marker < uprows-5; marker++) {
		...
		for (k=0; k<5; k++ ) {
			...
			gsl_fit_linear(cgh, 1, dperm, 1, 43, &mu, &alpha, &cov00, &cov01, &cov11, &sumsq);
...
int j;
for (j=0; j<BINCNT; j++ ) {
	//this only does 10 genes, change to 5*uprows*20997
	histogram_of_ratios[j]=histogram_of_ratios[j]/(5*(uprows-10)*10);
}

// turn into cummulative sum for p-val calculation
double cumsum;
cumsum=0;
//why did josh start at 1?
for (j=1; j<BINCNT; j++ ) {
