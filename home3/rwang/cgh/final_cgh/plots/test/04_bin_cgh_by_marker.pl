...
%cgh=();
#$file="../frag_size/g3cghnorm_and_pos.txt";
$file="../g3cghnorm_and_pos_hdr.txt";

open(HANDLE, $file) or die "cannot open cgh file\n";
$header=<HANDLE>; chomp $header;
@hd = split ("\t", $header); shift @hd; shift @hd; shift @hd;
foreach $cell (@hd) {
#for ($cell=1;$cell<84;$cell++){
	$cell =~ s/^c//g;
	$cell =~ s/^c//g;
	push (@hda, $cell); 
}
...
$file="../../../run3/hg18_retention_v2.txt"; 
#$file="pcr10.txt"; 
open(HANDLE,$file) or die "cannot open pcr file\n";
<HANDLE>;
...
while(<HANDLE>) {
	...
	for ($i=4; $i < $chrlength{$chrm}; $i++)  { 	
		#find the closest marker, then walk back 4 on one side, 6 on the other
	...
	if ( (scalar @{$closest[0]}) > 0 ){	
			...
			for ($m=0; $m< scalar @hda; $m++) {
				if ($chrm !~ /[XY]/){
					if ( avg(\@{@{$closest[$m]}}) > 0.0554) { print "1".$t; }  else  { print "0".$t; } 
				} else {
					if ( avg(\@{@{$closest[$m]}}) > 0.1132 ) { print "1".$t; }  else  { print "0".$t; } 
				}
			}	
