...
cgh=read.table("../g3cghnorm_and_pos_hdr.txt", header=T)
cghbin=read.table("binned1.txt", header=T)
centromere = read.table("hg18centromere.txt", header=T)
...
#plot centromere
		segments(centromere$chromStart[centromere$chrom==chr],-0.35, centromere$chromEnd[centromere$chrom==chr], -0.35, lwd=4, col="grey50")
	dev.off()
}
setwd(current)
