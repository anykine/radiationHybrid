	...
	for (my $i=0; $i<= $#matrix; $i++){
		if ($i< $NUMAUTOMARKERS){
			for (my $j = 0; $j< $NUMSAMPLES; $j++){
				if ($matrix[$i][$j] > $pthresh || $matrix[$i][$j] < $nthresh){
					$matrix[$i][$j] = 1;
				} else {
					$matrix[$i][$j] = 0;
				}
		...
		# Xchrom
			...
			for (my $j=0; $j<$NUMSAMPLES; $j++){
				if (defined $females{$j}){
					if ($matrix[$i][$j] > $fpthresh || $matrix[$i][$j] < $fnthresh){
						$matrix[$i][$j] = 1;
					} else {
						$matrix[$i][$j] = 0;
					}
				} else {
					if ($matrix[$i][$j] > $mpthresh || $matrix[$i][$j] < $mnthresh){
						$matrix[$i][$j] = 1;
					} else {
						$matrix[$i][$j] = 0;
					}
...
sub sumAcrossRows{
	...
	open(OUTPUT, ">cghmarkers.ok.x") || die "err $!" if $output;
	my @rowsum=();
