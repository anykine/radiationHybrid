...
int main(void) {
	...
	char cghfile[] = "allcgh.sort.1";
	char expr_file[] = "allexpr.sort.1";
	
	// ouptut file
	char output_file[] = "/media/usbdisk/tcga/regress_results_pval.txt";
...
fprintf(stderr,"loading markers per gene\n");
	...
	int junk;
	fi = fopen("../index/tcga_cis_markers.txt", "r");
	if (fi==NULL){
...
for (gene = 0; gene < exprgenes; gene++ ) { 
	...
	//for (marker = 0; marker < cghmarkers; marker++) {
		...
		r = gsl_stats_correlation(cgh, 1, expr, 1, 237);
		pval = corr_pvalue(r, CELLS-2);
		//printf("%d\t%d\t%.4lf\t%.4lf\t%.4lf\t%.4lf\n", gene+1, marker+1, mu, alpha, r, -log10(pval));
		fprintf(fo, "%d\t%d\t%.4f\t%.4f\t%.4f\t%.4e\n", gene+1, marker+1, mu, alpha, r, pval);
	}	
