...
unless (@ARGV==1){
	print <<EOH;
		...
		7/9/10 - modified to work with hapmap CNV data
		7/29/10 - modified to work with TCGA CNV data
EOH
exit(1);
}
...
#output first 4 rows of orig data (since its not smoothed) 
print OUTPUT $header;
for (my $i=0; $i<4; $i++){
	print OUTPUT join("\t", @{$matrix[$i]}), "\n";
}
...
# at any given time, matrix has 10 rows, 241 cols
sub average{
	my @newdata =();
	#use global matrix
	# 10 rows, 4cols(name,chr,start,stop), 237 cols of numbers
	for (my $i=4; $i < $NUMCOLS; $i++){
		my $sum=0;
	...
	# add the chrom,pos info
	unshift @newdata, $matrix[4][0], $matrix[4][1], $matrix[4][2], $matrix[4][3];
	return @newdata;
}
