...
sub assemble_cis_pos{
	my $file = shift;
	open(INPUT, $file) || die "cannot open cis $!";
...
unless (@ARGV == 1){
	print <<EOH;
	usage: $0 <file>
	eg $0 simple_cis.txt > all_cis_with_pos.txt

	Add the position and gene symbol information to the cis file.
EOH
exit;
}
load_gene_index();
#print Dumper(\%genepos);
assemble_cis_pos($ARGV[0]);
