...
void printhelp(char* prog){
	...
	printf(" -b1 <ascii> <binout> create binary file from ascii (with .bin)\n");
	printf(" \t expects 232626 rows, 20145 cols\n");
	printf(" -b2 <ascii> <binout> create binary file from ascii (with .bin)\n");
	printf(" \t expects 20145 rows, 232626 cols\n");
...
// 
// This creates a bin files 232626 rows, 20145 cols
// which is josh's orig formatting.
//-------------------------------------------
...
// create the BIN file from the ASCII
// 
// This creates a bin files 20145 rows, 232626 cols
// which is the tranpose of josh's orig formatting.
//-------------------------------------------
int convert2binlong(char* file, char* binout){
	int counter=0, counter2=0 ;
	FILE* fin = fopen(file, "r");
	if (fin==NULL) {
		err("cannot open input file");
	...
	//the multiplication by log10(3/2)/log10(2) has more sigfigs than
	//orig data, so the line buffer is made huge to prevent problems
	// storing 232626 values plus tabs, so this needs to be big
	char* line;
	line = malloc(3000001);
	if (line == NULL)
		err("malloc failed");
	...
	// read all rows in file, parse and output as binary
	for (row = 0; row < 20145; row++){ 
		fgets(line,3000000,fin); 
		pch = strtok (line, "\t");
		while (pch != NULL) {		
			value = atof(pch);
			//printf("%f\n", value);
			fwrite((void *)&value, sizeof(float), 1, fout);
			//look for either tab or newline
			pch = strtok (NULL, "\t\n");
			if (counter%232626==0){
				fprintf(stderr, "%d\n", counter2++);
			}
...
/**********************************************
 ...
 this only reads the 232626 rows,20145 col bin format
*********************************************/
...
int main(int argc, char* argv[]) {
	...
	int i;
	for (i=1; i<argc; i++){
		// 232626 rows, 20145 cols
		if (strcmp(argv[i], "-b1") == 0 ){
			printf("running convert on %s\n", argv[i+1]);
			convert2bin(argv[i+1], argv[i+2]);
			break;
		}
		// 20145 rows, 232626 cols
		if (strcmp(argv[i], "-b2") == 0 ){
			printf("running convert2binlong on %s\n", argv[i+1]);
			convert2binlong(argv[i+1], argv[i+2]);
			break;
		}
