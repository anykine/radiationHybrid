...
sub get_mus_hum_markers{
	open(INPUT, "/media/G3data/fdr18/trans/corr_huge/mus2human_closest.txt") || die "cannot open mus2hum\n";
	while(<INPUT>){
		chomp;
		my @line = split(/\t/);
...
get_common_gene();
print "*loaded common gene\n";
# get common markers
get_mus_hum_markers();
print "*loaded common markers\n";
#print Dumper(\%hum2musmarkers); exit(1);
my $counter=0;

# my file of hum & mus shared 0-gene eQTLs
foreach my $k (sort keys %hum2musmarkers){
...
#while(<INPUT>){
	...
	#chomp;
	#my @data = split(/\t/);
	print "start get human data for marker $k\n";
	my $humhashref = get_human_data_by_marker($k);
	print "end get human data for marker $k\n";
	#for (my $i=0; $i<20996; $i++){
	#	print $humhashref->{alpha}[$i],"\t";
	#	print $humhashref->{nlp}[$i],"\n";
	#}

	print "start get moue data $k\n";
	my $mushashref = get_mouse_data_by_marker($hum2musmarkers{$k});
	print "end get moue data $k\n";
