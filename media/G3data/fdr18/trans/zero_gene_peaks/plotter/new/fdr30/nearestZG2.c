...
int main(int argc, char* argv[]){
	...
	************************************/
	...
	while( fscanf(fpos, "%d %d %f %d %d %f", &mstart, &chromstart, &posstart, &mend, &chromend, &posend) != EOF){
		//printf("%d %d %f %f %s\n", muschr, humchr, start, end,buffer);
		printf("%d %d %f %d %d %f\n", mstart, chromstart, posstart, mend, chromend, posend);
		...
		*/
		...
		gdImageLine(im, hx, hy, hx+radius, hy, black); /*human*/
	...
	************************************/
	...
	while( fscanf(fpos, "%d %f %f", &chromstart, &posstart, &posend) != EOF){
		...
		gdImageLine(im, mx-radius, my, mx, my, black); /*mouse*/
	}
	fclose(fpos);
	...
	************************************/
	...
	while( fscanf(fpos, "%d %f %f", &chromstart, &posstart, &posend) != EOF){
		...
		gdImageRectangle(im, hx-radius, ycoord1, hx+radius, ycoord2, grey);
	}
	gdImagePng(im,fout);
	gdImageDestroy(im);
	fclose(fout);
