...
use DBI;
use Data::Dumper;
# map cgh marker to closest gene (using microaray gene list)
...
#
# 6/24/08 - modified output: closest-gene-to-marker-index and num genes
#  regulated; compare regulators between mouse/human
# 9/9/10 - use this to get FDR30
my $BED = 0;
...
sub find_closest_gene{
	...
	return($sym,$def,$ont,$chr,$start,$stop,$closestidx);
}
...
sub load_markers{
	open(INPUT, "../regulator_countFDR30.txt") || die "cannot open file1\n";
	while(<INPUT>){
		chomp;
		my @data = split(/\t/);
