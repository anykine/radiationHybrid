#!/usr/bin/perl -w
#
# For a lincRNA that overlaps a (mouse) zero gene block,
...
sub find_closest_cgh_to_lincRNA{
	my %closeCGH=();
	load_markerpos_from_db_range("mouse_rhdb");
	open(INPUT, "../../20090330mus_lincRNA_overlap.txt") || die "cannot open mouse linc overlap";
	while(<INPUT>){
		next if /^#/; chomp;
		my @d = split(/\t/);
		my($chr,$lincStart,$lincStop) = ($d[0],$d[3], $d[4]);
		my %best=(dist=> 10000000);
		for (my $i=0; $i<scalar @{$t31markerpos{$chr}{pos}}; $i++){
			if (abs($lincStart - ${$t31markerpos{$chr}{pos}}[$i]) < $best{dist}){
				$best{dist} = abs($lincStart-${$t31markerpos{$chr}{pos}}[$i]);
...
#load_mus_blocks();
#matchup();
my $close = find_closest_cgh_to_lincRNA();
