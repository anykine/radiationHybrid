...
sub build_amon_genes{
	my $counter=0;
	open(INPUT, "chr_amon.txt") || die "cannot open among data\n";
	while(<INPUT>){
		chomp;
		my @data = split(/\t/);
		...
		foreach my $k (@symbols){
			...
			if (defined $amon_genes{$k} ){
				#print "$k exists!\n";
				my $runavg = ($amon_genes{$k}{count} * $amon_genes{$k}{alpha} + $data[5])/($amon_genes{$k}{count} +1);
				$amon_genes{$k}{alpha} = $runavg;
				$amon_genes{$k}{count}++;
			} else {
				#key = symbol, val=gene number
				$amon_genes{$k}{alpha} = $data[5];
