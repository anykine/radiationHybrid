...
open(INPUT, "g3alpha_model_results1_gt3.2trans.txt") || die "cannot open big file\n";
#open(INPUT, "g3alpha_model_results1_gt2.4trans.txt") || die "cannot open big file\n";
while(<INPUT>){
	chomp;
	my @line = split(/\t/);
	print $genepos[$line[0]-1], "\t", $markerpos[$line[1]-1], "\t", $line[4], "\n";
}
