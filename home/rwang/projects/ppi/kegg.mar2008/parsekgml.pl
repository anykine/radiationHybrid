...
#twig stuff
my $twig_handlers = {'entry' => \&entry_handler, 'relation' => \&relation_handler};
my $twig = new XML::Twig(TwigRoots=>{'pathway' => 1}, TwigHandlers => $twig_handlers);
...
#if ($file){
#	open(OUTPUT, ">./$file".".out")or die "cannot open file for output\n";
#	$twig->parsefile($file);
#} else {
	chdir "./human" || die "cannot open dir\n";
	while($file=<*.xml>){
		open(OUTPUT, ">./$file".".out")or die "cannot open file for output\n";
		print $file,"\n";
		$twig->parsefile($file);
...
sub entry_handler{
	...
	} elsif (($tag->att('type') eq 'ortholog') && !(defined $tag->att('map'))) {
		$entryref->{$tag->att('id')} = {
			keggids=> [ split(/ /, $tag->att('name')) ],
			xcoord => $tag->first_child('graphics')->att('x')
		};
	# these entries are NOT entry or relation type
		...
		print "unhandled entry: ", $tag->att('type'), "\n" if $DEBUG;
		#print $tag->att('type'),"\n" if $DEBUG;
	}
}
sub relation_handler{
	...
	if (defined $tag->att('entry1') && defined $tag->att('entry2')){
		...
		if ((exists $entryref->{$tag->att('entry1')} ) && 
			...
			print "relation: entry1=",$tag->att('entry1') ," entry2=", $tag->att('entry2'), "\n";
			push @entries, $tag->att('entry1'), $tag->att('entry2');

			#if there's no subtype, this causes a problem...
			if ($tag->children('subtype')) {
				@reltypes = $tag->children('subtype') ;
			} else {
				print "no subtype, escaping...\n";
				return;
			}
			...
			if ($#reltypes > 0){
				...
				$reltypes = join("|",@types);
			} elsif (defined $reltypes[0]->att('name'))  {
