...
# yes this is a hack.. Sliding window of 5 and 10 basepairs.
use strict;
use Data::Dumper;
...
#loop from 0..59
for (my $i=0; $i<=59; $i++){
	walkMatrix(\@arrgraphics, \@probenames, $i, 1);
}
#walk the Nx60 matrix by cols
#for pos=1..60, find probes w/mm at pos and print avg rval. do same for probes w/match
sub walkMatrix{
	my($aref, $proberef, $start, $windowsize) = @_;
	my @matchprobes = ();
	my @mmatchprobes = ();
	my $match=0;
	my $mmatch=0;
	#window fits < 60 bp (0..59)
	if ($start <= 59 - $windowsize + 1){
		for (my $i=$start; $i<= $start+$windowsize-1; $i++){
			for (my $j=0; $j<= $#$aref; $j++){
				if ($aref->[$j][$i] eq '|') {
					$match++;
	...
	#if window extends beyond 60bp, wrap to front
	} else {
		my $wrap = $start - (59 - $windowsize +1);
		my $i;
		for ($i=$start; $i<= 59; $i++){
			for (my $j=0; $j<= $#$aref; $j++){
				if ($aref->[$j][$i] eq '|') {
	...
	print "$start\t$resmm\t$resmmse\t$mmatch\t$resm\t$resmse\t$match\n";
}
